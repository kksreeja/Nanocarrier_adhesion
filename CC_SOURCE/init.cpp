...
do {
    ...
    while (!accpflag){
      ...
      meanz = module_datastruct_mp_compute_closest_vertex(&ii, &xposnc,&yposnc);
...
do {
    ...
    while (!accpflag){
      int ncounter = 0;  
      double dshift = 0.0;
      xposnc = mtrand1.rand(0.2*this->L,0.8*this->L);
      yposnc = mtrand1.rand(0.2*this->L,0.8*this->L);
      meanz = module_datastruct_mp_compute_closest_vertex(&ii, &xposnc,&yposnc);
      zposnc = min(this->H - nc.getsoft_radius(i), meanz + roffset);                               // choose a position between membrane and max box height
      ...
      } while ((ncounter<1000000) && !accpflag);                                            // If fails then try a new XY position  
    }
    cout<<"NC "<<i<<"(getx,y,z): "<<nc.getxc(i)<<" "<<nc.getyc(i)<<" "<<nc.getzc(i)<<endl;
    i++;
