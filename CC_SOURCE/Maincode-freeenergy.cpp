...
int main(int argc, char* argv[]) {
  ...
  module_datastruct_mp_processor_number = processor;
  window=processor;
  ...
  if ((argc > 1) && (strcmp(argv[1],"true")==0)) {
    ...
    debug_mode = true;
    module_datastruct_mp_debug_mode = true;
  ...
  stringstream s1,s2,s3,s44,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13;
  ofstream nc_angle,nc_ab_data,antigen_data,multivalency_file,zhist_file,Hhist_file,ti_ener; 
  ofstream nc_valency_angle, re_Position_file,re_BPosition_file,re_UBPosition_file,re_Probability_file;

  //------->@ Buffer the data written to files
  char *buffer2,*buffer4,*buffer5,*buffer6,*buffer7,*buffer8,*buffer9,*buffer10,*buffer11,*buffer12;
  buffer4 = new(nothrow) char[BUFFER_SIZE_SMALL];
  buffer2 = new(nothrow) char[BUFFER_SIZE_LARGE];
  buffer9 = new(nothrow) char[BUFFER_SIZE_LARGE];
  buffer10 = new(nothrow) char[BUFFER_SIZE_LARGE];
  buffer11 = new(nothrow) char[BUFFER_SIZE_LARGE];
  buffer12 = new(nothrow) char[BUFFER_SIZE_LARGE];
  
  nc_valency_angle.rdbuf()->pubsetbuf(buffer4,BUFFER_SIZE_SMALL);
  antigen_data.rdbuf()->pubsetbuf(buffer2,BUFFER_SIZE_SMALL);
  re_Position_file.rdbuf()->pubsetbuf(buffer9,BUFFER_SIZE_LARGE);
  re_BPosition_file.rdbuf()->pubsetbuf(buffer10,BUFFER_SIZE_LARGE);
  re_UBPosition_file.rdbuf()->pubsetbuf(buffer11,BUFFER_SIZE_LARGE);
  re_Probability_file.rdbuf()->pubsetbuf(buffer12,BUFFER_SIZE_LARGE);

  int  ant_bound_flag, antigen, datastep;
  struct nanocarrier *ncpointer;
  ncpointer =&module_datastruct_mp_nc_f;
  ...
  if (module_datastruct_mp_memb_restart_flag == false && RP.DTMCflag){      
   if (debug_mode) cout << str_blue<<"Equilibrating membrane "<<endl;
    ...
    for (int dtmcs = 0 ; dtmcs < RP.DTMCsteps; dtmcs++){
      ...
      if((dtmcs >= halfDTMC) && (dtmcs%RP.conf_freq == 0)){
         ccstep += 1;
        // module_writedata_mp_write_membrane_xyz(&RP.ensno,&twindow,&ccstep);
         module_writedata_mp_write_membrane_area(&RP.ensno,&twindow,&ccstep);       // print membrane area at this interval
         if (ccstep%100 == 0){
  ...
  int num_ab[num_nanocarrier];
    ...
    // Inputs for Receptors  
  int *antigen_Counter;
  double *pp_ant;
  bool *index_ant;

   antigen_Counter = new(nothrow) int[num_antigens];
   pp_ant = new(nothrow) double[num_antigens];
   index_ant = new (nothrow) bool[num_antigens];

   fill_n(antigen_Counter,num_antigens,0);
   fill_n(pp_ant,num_antigens,0.0);
   fill_n(index_ant,num_antigens,false);
  ...
  s3<<"Re_Position-"<<processor<<"_Frame-"<<window<<".dat";
  re_Position_file.open(s3.str().c_str(), ios::out);

  s11<<"Re_BPosition-"<<processor<<"_Frame-"<<window<<".dat";
  re_BPosition_file.open(s11.str().c_str(), ios::out);

  s12<<"Re_UBPosition-"<<processor<<"_Frame-"<<window<<".dat";
  re_UBPosition_file.open(s12.str().c_str(), ios::out);

  s13<<"Re_Probability-"<<processor<<"_Frame-"<<window<<".dat";
  re_Probability_file.open(s13.str().c_str(), ios::out);

  s2<<"Antigen-Data-ENS-"<<processor<<"_Frame-"<<window<<".dat";                                                       // bonddist,theta,phi for the antigen
  antigen_data.open(s2.str().c_str(), ios::out);
 
  s4<<"NC-multivalency-angle-ENS-"<<processor<<"_Frame-"<<window<<".dat";
  nc_valency_angle.open(s4.str().c_str(), ios::out);

  RP.sampling_cutoff = 0;
  ...
  for (window = RP.win_start; window < RP.win_end; window++){
   ...
   //@ Monte Carlo dynamics 
    for (int curr_step = RP.step_start; curr_step < RP.step_end; curr_step++){        // Production run ( a pre-equilibrated membrane has been used here)
      module_datastruct_mp_curr_step= curr_step;                                    // if in case you like to access current step number from fortran
      ...
      if (curr_step >= RP.sampling_cutoff){
       ...
       //  w.write_antigen_xyz(processor,window,datastep);
        // module_writedata_mp_write_membrane_xyz(&RP.ensno,&window,&datastep);
         module_writedata_mp_write_membrane_area(&RP.ensno,&window,&datastep);       // print membrane area at this interval
...
//      counter for bound states of each antigen at each time step.        
                int ab = 0;
                for (int i=0;i<num_nanocarrier;i++){
                  while (ab<num_ab[i]){
                        curr_multivalency = nc.get_multivalency(i);      // we can estimate that how many of Res get bonded

                    for (int j=0;j<num_antigens;j++) {
                       if (nc.getbonded(i,ab)){
                         if (j == nc.getbondedto(i,ab)){
                             antigen_Counter[j] = antigen_Counter[j] + 1;
                       //   cout<<"ab = "<<ab<<", j= "<<j<<", ant = "<<nc.getbondedto(i,ab)<<", antigen_Counter = "<< antigen_Counter[j]<<endl;         
...
//      counter for bound states of each antigen at each time step. 
        ...
        if((curr_step%RP.antab_freq==0)){
          datastep = (int) curr_step/RP.antab_freq;
          for (int i=0;i<num_nanocarrier;i++){                                                                     //output the antigen bond length and angle
           curr_multivalency = nc.get_multivalency(i);
           nc_valency_angle<<datastep<<" "<<i<<" "<<curr_multivalency<<" "<<setprecision(9)<<nc.getanglex(i)<<" "<<
           setprecision(9)<<nc.getangley(i)<<" "<<setprecision(9)<<nc.getanglez(i)<<endl;
           
           bound_flag=0;
           for (int j=0;j<num_ab[i];j++) {
             if (nc.getbonded(i,j)){
               if (bound_flag==0){                                                                     // write the time step and vesicle number only  once
                 antigen_data<<datastep<<" "<<i<<" ";
                 bound_flag=1;
                }
                antigen = nc.getbondedto(i,j);
                antigen_data<<antigen<<" " << nc.compute_bond_length(i,j,antigen)<<" ";
                antigen_data<<re.get_theta(antigen)<<" "<<re.get_phi(antigen)<<" ";                   // the computed values are stored in the antigen pointer structure
                ant_bound_flag=1;
              }
             } 
      } 
         if (ant_bound_flag == 1) antigen_data<<endl;
     }   
 
     //@ Write the bead and Mem position data and also use it for Entropy calculation
                 if((curr_step%RP.hist_freq==0)){

                     re_Position_file<<curr_step<<endl;

                   for (int i=0; i<num_antigens; i++){
                       re_Position_file<<i+1<<"      "<<re.getxt(i)<<"      "<<re.getyt(i)<<"      "<<re.getzt(i)<<endl;
                    }

                   re_Probability_file<<curr_step<<endl;
                   int interval = curr_step - RP.sampling_cutoff + 1;

                  for (int i=0; i<num_nanocarrier; i++){
                    for (int j=0; j<num_antigens; j++){

                     pp_ant[j] = (double) antigen_Counter[j]/(double) interval;  // curr_step; 

                     re_Probability_file<<j+1<<"      "<<pp_ant[j]<<endl;
                    }
                  }

//      Position of bound receptors with thier index

                re_BPosition_file<<curr_step<<endl;

                for (int i=0; i<num_nanocarrier; i++){
                  curr_multivalency = nc.get_multivalency(i);
                  if(curr_multivalency != 0) {
                  //  re_BPosition_file<<i<<" "<<curr_multivalency<<" ";   // pos and bonded antibody pos on the NC
                    for (int j=0; j<num_ab[i]; j++) {
                      if (nc.getbonded(i,j)==1){
                        antigen = nc.getbondedto(i,j);
                        index_ant [antigen] = 1;
                        re_BPosition_file<<antigen+1<<" "<<re.getxt(antigen)<<" "<<re.getyt(antigen)<<" "<<re.getzt(antigen)<<endl;
                      }
                    }
                   }
                  }

                  // position of unbound receptors when some of them are bound to the ab

                   re_UBPosition_file<<curr_step<<endl;
                     for (int i=0; i<num_nanocarrier; i++){
                        for (int j=0; j<num_antigens; j++) {
                          if (index_ant[j] == 0){
                            antigen = j;
                            re_UBPosition_file<<antigen+1<<" "<<re.getxt(antigen)<<" "<<re.getyt(antigen)<<" "<<re.getzt(antigen)<<endl;
                           }
                          }
                         }
       ...
       if((curr_step%RP.antab_freq==0)){
		...
		membrane_energy[syst_array_counter] = module_datastruct_mp_total_membrane_energy();  
     ...
     nc_valency_angle.close(); s4.str(""); 
    antigen_data.close(); s2.str("");
    re_Position_file.close();s3.str("");
    re_BPosition_file.close();s11.str("");
    re_UBPosition_file.close();s12.str("");
    re_Probability_file.close();s13.str("");
    ...
    if ( RP.system.compare("DTMC_membrane") != 0) {
		...
		s44<<"NC-angle-ENS-"<<RP.ensno<<"_Frame-"<<window<<".dat";
		nc_angle.open(s44.str().c_str(), ios::out);
		...
		nc_angle.close(); s44.str("");                                                        // close the file
		//--------------------------------->
