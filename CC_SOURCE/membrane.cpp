...
void _MEMBRANE ::init() {
  module_makesurface_mp_makesurface();                   // call to build the membrane
  this->nver  =  module_datastruct_mp_nver;              // store the parameters
  this->pbnum =  module_datastruct_mp_pbnum;
  this->ntr  =  module_datastruct_mp_ntr;
  this->nlink  =  module_datastruct_mp_tlink;
...
void _membrane :: movevertex(bool eqmflag){
  if (eqmflag)
    module_mcsmoves_mp_link_flip();
  else
  module_mcsmoves_mp_vertex_move_biased();
}

void _membrane :: fliplink(bool eqmflag){
  if (eqmflag)
    module_mcsmoves_mp_vertex_move();
  else 
   module_mcsmoves_mp_link_flip_biased();
}

void _membrane :: membrane_montecarlo(bool eqmflag){
  if (eqmflag)
    module_mcsmoves_mp_membrane_montecarlo();
  else
    module_mcsmoves_mp_membrane_montecarlo_biased();
}

void _membrane :: dump_membrane_conf(int ensno){
	module_writedata_mp_membrane_dump1(&ensno);                                    
}

void _membrane :: dump_membrane_conf(int ensno, int window){
	module_writedata_mp_membrane_dump2(&ensno,&window);                                    
}
