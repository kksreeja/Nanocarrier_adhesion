...
void _NANOCARRIER :: init(int i){
  ...
  this->verptr = &module_datastruct_mp_ver;
  this->ncpointer= &module_datastruct_mp_nc_f + this->index;    //@ Point to the corresponding NC.
...
void _NANOCARRIER :: init(int ncindex, ifstream &ifile) {
	...
	this->verptr = &module_datastruct_mp_ver;
	this->ncpointer= &module_datastruct_mp_nc_f + this->index;    //@ Point to NC.
	this->setnum_ab();
...
void _nanocarrier :: set_array_sizes(){
	...
	this->verptr = &module_datastruct_mp_ver;
	this->triptr = &module_datastruct_mp_tri;
	this->antptr = &module_datastruct_mp_antig;
}
...
void _nanocarrier :: store_restore_shadow(int flag, int ncnum){
  module_datastruct_mp_store_restore_shadowvertices(&flag,&ncnum);
}

void _nanocarrier :: compute_shadow(int ncnum){
  module_datastruct_mp_compute_nc_shadow_vertices(&ncnum);
}
...
tmatrix _nanocarrier :: construct_householder_matrix(int sel_antigen){
  ...
  if (anttri != -1){
   char marker='f';
   module_curvcalc_mp_compute_householdermatrix(&fanttri,&marker);   // compute the householder matrix for the face anttri ('f' is marker for triangle)
   for (int i=0; i<3; i++){
  ...
  {
    char marker='n';
    module_curvcalc_mp_compute_householdermatrix(&fantvert,&marker); // compute the householder matrix for the vertex antvert ('n' is marker for node)
    for (int i=0; i<3; i++){
