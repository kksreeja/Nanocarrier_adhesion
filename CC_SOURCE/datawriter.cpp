...
//    this->eqm_bond_dist=data.geteqm_bond_dist();
    ...
    this->verptr=&module_datastruct_mp_ver;                // Initialize pointers to vertex,triangle and antigen 
    this->triptr=&module_datastruct_mp_tri;
    this->antptr=&module_datastruct_mp_antig;
    this->ncpointer=&module_datastruct_mp_nc_f;
}
...
void _datawriter:: write_antigen_xyz(int rank,int window,int step){
	...
	fs<<"#boxlen:  "<<(float)this->boxl<<"  area:  "<<(float)module_datastruct_mp_get_membrane_area()<<endl;
...
void _datawriter :: _VTK_membrane(ofstream &of1)
{
	...
	for(i=0; i<this->num_ves; i++){                                             //get the center of mass position of the membrane under the shadow of nanocarrier i
		int j = i+1;
		module_datastruct_mp_ncshadow_r(&j);
	}
