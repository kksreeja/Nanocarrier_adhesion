...
//->
  ...
  if ((new_vertex != vertex_index) && ((this->verptr+new_vertex)->nantigen < maxantigen_ver)){  
	...
	char ch='f';
	module_curvcalc_mp_compute_householdermatrix(&triang_no,&ch);
	...
	char ch = 'n';
	module_curvcalc_mp_compute_householdermatrix(&vertex_num,&ch);
...
_receptor :: _receptor(double L1, double H1, int num_antigens1){
  ...
  this->num_memb_vert=module_datastruct_mp_nver;
...
void _receptor ::receptor_diffusion(){
  module_mcsmoves_mp_antigen_diffusion_on_triangle();
}

void _receptor ::receptor_hopping(){
  module_mcsmoves_mp_antigen_hopping_on_vertex();
}
