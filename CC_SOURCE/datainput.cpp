...
void _datainput :: init(){
  ...
  kBT = kb * this->temperature;                                                                                                      
   module_datastruct_mp_blen = this->periodic_box_length/(double)(this->N+1);             // scale box size with initial blength for setting up the membrane
...
void _datainput :: read_system_geometry(){
  ...
  this->hist_samp_freq = parse.PARSEINT(filename, DELIMITER, "hist_samp_freq");
...
void _datainput :: dump_system_geometry(ofstream &ofile){
	...
	ofile<<"hist_samp_freq "<<this->hist_samp_freq<<endl;
...
void _datainput :: read_system_geometry(ifstream &ifile){
	...
	ifile>>temps>>this->hist_samp_freq;
...
void _datainput :: check_periodicbox_dimensions(){
  ...
  if (boxflag==1){
   ...
   if (debug_mode)
      cout <<"\n \t\t\t\t Rescaling average bond distance to "<<module_datastruct_mp_blen<<endl;
  }
...
void _datainput :: setup_runparameters(){
  ...
  this->rparam.hist_freq = this->hist_samp_freq;
}
...
int _datainput ::  getnum_members(char ch){
 ...
 if (ch =='c')
   return num_antigens;
 if (ch == 'm')
   return module_datastruct_mp_nver;
 return 0.0;
}
...
void _datainput ::  setnum_members(char ch, int nval){
	...
	if (ch =='c')
		num_antigens = nval;
	if (ch == 'm')
		module_datastruct_mp_nver = nval;
}
...
int _datainput :: get_hist_samp_freq(){
  return hist_samp_freq;
}
...
void _datainput :: initialize_fortran_variables(){
  module_datastruct_mp_blen = memb_init_bond_length;                        // scale box size with initial blength for setting up the membrane
  module_datastruct_mp_gsize = getN()+1;                                    // Pass the grid size to the fortran subroutine (0 -> N) for fortran
  module_datastruct_mp_num_antigens = getnum_members('c');                  // Pass the number of antigen to the fortran
  module_datastruct_mp_num_nanocarrier = getnum_members('v');               // Pass the number of nanocarriers to the fortran
  module_datastruct_mp_periodic_box_length =getperiodic_box_length();       // Pass the box length
  module_datastruct_mp_periodic_box_height =getperiodic_box_height();       // Pass the box height
  module_datastruct_mp_membrane_init_z = getmembrane_init_z();              // Initial height at which the membrane is initialized
  module_datastruct_mp_beta = (double) 300.0/gettemperature();
}
