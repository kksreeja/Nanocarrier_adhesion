...
#for syst in ['SB','IB','WB']:
for syst in ['IB','WB']:
    #for rmode in ['approach','recede']:
    for rmode in ['approach']:
        ...
        ['sampling_interval',int(num_mc_moves/1000000)],
        ['window_antab_data_interval',int(num_mc_moves/100000)],
        ...
        blen_fac=[1.25]
        num_antibody = [162]
        zvalues=zstartdict[rmode]
        
        startconf_dir=source_dir+'/../..'
        codehome=os.getcwd()
        exec_name='free_ener'

        dirname0 = 'kspring-1.0'
        subprocess.call(["mkdir","-pv",dirname0])
        os.chdir(dirname0)
        
        
        run_prefix=syst+'-Rbias-'+rmode+'-'
    		...
    		os.chdir(dirname2)
       		...
       		for kappa in kappaval:
        		...
        		for ens in range(start_ens,end_ens):
        		    ...
        		    fp.write("nohup mpirun -np "+str(n_processor)+" ./"+str(run_exe)+'  false \n')
        		    fp.close()
