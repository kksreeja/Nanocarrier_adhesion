...
!---------------------------------------------------------------------------------------------------------------------------
	...
	Implicit None
	Include '_mod_mcsmoves_interface.h'
	Integer :: i,j,k,vert,k1,antigen_no,vj
	Real (Kind =8 )::energy

       Do j=1,ver(vert)%nonei+1,1
       if(j.gt.ver(vert)%nonei)then
       vj=vert
       else
       vj=ver(vert)%vneipt(j)
       endif
       ver(vj)%czero=0
       ver(vj)%czero_flag=0
       mp%vertex(vj)%czero=0
       mp%vertex(vj)%czero_flag=0
       Do i=1,ver(vj)%nantigen,1
       antigen_no=ver(vj)%antigen_list(i)
       If((bondedstate(antigen_no-1) .Eqv. .TRUE.))then
       ver(vj)%czero=czero
       ver(vj)%czero_flag=1
       mp%vertex(vj)%czero=czero
       mp%vertex(vj)%czero_flag=1
       endif
       Enddo
      Enddo 
...
!---------------------------------------------------------------------------------------------------------------------------
	...
	Implicit None
	Include '_mod_mcsmoves_interface.h'
	Integer :: i,j,k,vert(4),k1,vi,vj,check,antigen_no
	Real (Kind =8 )::energy
	energy = 0.0
...
!---------------------------------------------------------------------------------------------------------------------------
    ...
    Integer :: i,j,vj,check,vi,k,antigen_no
    Real(Kind=8) :: flen
    flen=0
...
!------------------------------------------------------------------------------------------------------------------------
    ...
    IMPLICIT NONE
    Include '_mod_mcsmoves_interface.h'
    Integer :: v,i,vno,antigen_no,j,vj
    Real(Kind=8) :: mven
    mven=0

    Do j=1,ver(vno)%nonei+1,1
       if(j.gt.ver(vno)%nonei)then
       vj=vno
       else
       vj=ver(vno)%vneipt(j)
       endif
